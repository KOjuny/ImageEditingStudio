...
import numpy as np
import abc
from models.p2p import ptp_utils
from models.p2p import seq_aligner_p2p
...
class AttentionReplace(AttentionControlEdit):
    ...
    def __init__(self, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
        ...
        self.mapper = seq_aligner_p2p.get_replacement_mapper(prompts, tokenizer).to(device)
...
class AttentionRefine(AttentionControlEdit):
    ...
    def __init__(self, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
        ...
        self.mapper, alphas = seq_aligner_p2p.get_refinement_mapper(prompts, tokenizer)
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='/home/poong/junseok/ImageEditingStudio/evaluation/output/p2p', help='Directory to save output images')
    parser.add_argument('--prompt_dir', type=str, required=True, help='List of text prompts for image generation')
    ...
    for prompt_set in sets_of_prompts:
        ...
        for i in range(1,len(prompts)):
            ...
            if not os.path.exists(os.path.join(output_folder, "generated_image_0.png")):
                img_0 = Image.fromarray(images[0])
                img_0.save(os.path.join(output_folder, "generated_image_0.png"))
            img_path = os.path.join(output_folder, f"generated_image_{i}.png")
            img_1 = Image.fromarray(images[1])
            img_1.save(img_path)
