...
def p2p_guidance_forward(
    ...
    return latents, latent # reconstruction latent, x_t
...
def direct_inversion_p2p_guidance_forward(
    ...
    latent, latents = init_latent(latent, model, height, width, generator, batch_size)  # latent : torch.Size([1, 4, 64, 64]), latents : torch.Size([2, 4, 64, 64])
    model.scheduler.set_timesteps(num_inference_steps)
    for i, t in enumerate(model.scheduler.timesteps):
