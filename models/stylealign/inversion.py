...
class NullInversion:
    ...
    def get_noise_pred_single(self, latents, t, context):
        pdb.set_trace()
        noise_pred = self.model.unet(latents, t, encoder_hidden_states=context)["sample"]   # 여기서 True로 바뀜 context : torch.Size([1, 77, 768])
        return noise_pred
        ...
        for i in range(self.num_ddim_steps):
            ...
            if num_inner_steps!=0:
                ...
                for j in range(num_inner_steps):
                    noise_pred_uncond = self.get_noise_pred_single(latent_cur, t, uncond_embeddings)    # True
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond) # True
                    latents_prev_rec = self.prev_step(noise_pred, t, latent_cur)    # True
                    loss = nnf.mse_loss(latents_prev_rec, latent_prev)  # True
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                    loss_item = loss.item()
...
class DirectInversion:
    ...
    def offset_calculate(self, latents, num_inner_steps, epsilon, guidance_scale):
        noise_loss_list = []
        latent_cur = torch.concat([latents[-1]]*(self.context.shape[0]//2)) # torch.Size([2, 4, 64, 64])
        for i in range(self.num_ddim_steps):            
            latent_prev = torch.concat([latents[len(latents) - i - 2]]*latent_cur.shape[0]) # torch.Size([2, 4, 64, 64]) -> inversion을 통해 얻은 latent z^*
            t = self.model.scheduler.timesteps[i]
            with torch.no_grad():
                noise_pred = self.get_noise_pred_single(torch.concat([latent_cur]*2), t, self.context)  # torch.Size([4, 4, 64, 64])
                noise_pred_uncond, noise_pred_cond = noise_pred.chunk(2)    # 각각 torch.Size([4, 4, 64, 64])
                noise_pred_w_guidance = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)
                latents_prev_rec, _ = self.prev_step(noise_pred_w_guidance, t, latent_cur)  # 예측한 latent z^''
                loss = latent_prev - latents_prev_rec   # 두 latent의 차이
                
            noise_loss_list.append(loss.detach())   # 차이 저장
            latent_cur = latents_prev_rec + loss    # 차이를 더해줘서 그걸로 update
    ...
    def invert(self, image_gt, prompt, guidance_scale, num_inner_steps=10, early_stop_epsilon=1e-5):
        ...
        image_rec, ddim_latents = self.ddim_inversion(image_gt) # image_rec : (512,512,3)크기의 image, ddim_latents : 51 * torch.Size([1, 4, 64, 64])
